...
2. 在对应期数里面 (比如 issue-2 , 代表第二期) 里面创建一个readme.md文件，文件命名格式为: 中文标题名，比如Android MVP模式与实践;
3. 将模板[template.md](template.md)中的内容拷贝到readme.md，按照模板填写、翻译完内容(文章所需图片请存放到图床上，不要放在仓库中,图片宽度尽量控制在400 px左右),完成翻译后将状态修改为"校对中",提交pull request到本项目;
4. 管理员校对完成之后便会发布.   
